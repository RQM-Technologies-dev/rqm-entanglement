...
def _mul(a: str, b: str) -> tuple[complex, str]:
    if a == "I":
        return 1, b
    if b == "I":
        return 1, a
...
def _rotation_table(axis: str) -> tuple[NDArray[np.int_], NDArray[np.int_], NDArray[np.float64]]:
    source = []
    target = []
    sign = []
    for i, a in enumerate(_LABELS):
        for j, b in enumerate(_LABELS):
            if sum(x != "I" and x != axis for x in (a, b)) % 2:
                ...
                pb, bb = _mul(axis, b)
                source.append(4 * i + j)
                target.append(4 * _LABELS.index(aa) + _LABELS.index(bb))
...
def _cx_table() -> tuple[NDArray[np.int_], NDArray[np.float64]]:
        ...
        for b in _LABELS:
            ...
            for q, label in enumerate((a, b)):
                ...
                if label == "Y":
                    phase *= 1j
                for kind in kinds:
                    mapped = generators[kind, q]
                    p, x = _mul(result[0], mapped[0])
                    r, y = _mul(result[1], mapped[1])
                    phase *= p * r
                    result = x + y
            target.append(4 * _LABELS.index(result[0]) + _LABELS.index(result[1]))
            sign.append(float(phase.real))
...
def apply_pair_coefficients(
    coefficients: ArrayLike, axes: Sequence[int], gate: str, angle: float = 0.0
) -> NDArray[np.float64]:
    """Apply a hinge or CX to two coefficient axes; CX axes are control,target."""
    tensor = np.asarray(coefficients, dtype=float)
    pair_axes = list(axes)
    order = pair_axes + [q for q in range(tensor.ndim) if q not in pair_axes]
    moved = tensor.transpose(order)
    flat = moved.reshape(16, -1)
    if gate == "cx":
        target, sign = _CX
        out = np.empty_like(flat)
        out[target] = sign[:, None] * flat
    elif gate in ("rxx", "ryy", "rzz"):
        source, target, sign = _ROT[gate[1].upper()]
        out = flat.copy()
        out[source] = math.cos(angle) * flat[source]
        out[target] += math.sin(angle) * sign[:, None] * flat[source]
    else:
        raise ValueError(f"unsupported analytic pair gate {gate}")
